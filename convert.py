...
tree = ElementTree()
tree.parse(open(args.filename, "r", encoding='utf-8'))
...
for node in nodes[:]:
	if not args.static:
		out["nodes"][node.get("id")] = { "label": getattr(node.find(graphml.get("data")), "text", "") }
	else:
		out["nodes"][node.get("id")] = {
			"label": getattr(node.find(graphml.get("data")), "text", ""),
...
open(outfilename, "w").write(json.dumps(out, indent=4))